...
- **Language as a domain**: the business domain is multilingual; every row that carries user-visible text, and every function that produces it, carries a `locale` field or parameter from the start. Relation rows and official labels used as identifiers (such as a Cineca curriculum name) do not — `docs/decisioni.md`, 2026-09-25, *The data model is decided on paper*, point 6. No hard-coded language strings.
