...
- **当前阶段 🔶 M2**：骨架就位 — `pyproject.toml`（uv · ruff · pyright · pytest 配置集中于此）、Django project `config/`、空包 `rag/`、`tests/test_smoke.py`、`.pre-commit-config.yaml`、`.github/workflows/ci.yml`。无 RAG 与 web 业务逻辑。
