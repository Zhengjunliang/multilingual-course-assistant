...
- **当前阶段 🔶 M2**：骨架（`pyproject.toml` 集中 uv · ruff · pyright · pytest 配置、Django project `config/`、`.pre-commit-config.yaml`、`.github/workflows/ci.yml`）+ ingest 前两步 —— `rag/probe.py`（解析前探测，逐份文件路由到经典 / 经典+公式富化 / VLM）与 `rag/parse.py`（Docling 解析）。无 chunking、检索、生成与 web 业务逻辑。
