...
1. **Each app keeps a single `0001_initial.py` until the first database whose data has to be kept.** A model change rewrites that file — it is deleted and generated again — and the local database is rebuilt; nothing is added as a `0002`. From the first database that must keep its data, such as a demonstration on the MICC servers or a deployment, migrations only add, as point 5 of *The data model is decided on paper* (2026-09-25) states. This narrows that point to the time after such a database exists; its other half, that each table lands with the issue that first uses it, is unchanged.
   **Why:** a migration file carries a database that already holds data from one schema to the next. With no such database there is nothing to carry, and every extra file is history nobody will replay. The files cannot go altogether: Django requires the custom user model to be created in the first migration of its app, and apps without migrations must not have relations to apps with them (outside the repository: Django 5.2 documentation, "Customizing authentication" and "Migrations"). Collapsing the migrations before the first release is what InvenTree did before its 1.0 (outside the repository: InvenTree issue 5015 and pull request 12830), and Andrew Godwin, who wrote Django's migration framework, described squashing as meant for installs one cannot control (outside the repository: django-developers, "Do people actually squash migrations?"). Point 1 of 2026-08-24 already counted on this window when it switched to a custom user model while doing so was free.
